...
def create_tasks():
    if request.method == 'POST':
        ...
        if not request.cookies.get('appName'):
            print("No cookie 'appName' is found")
        else:
            print("Cookie 'appName' is found : " + request.cookies.get('appName'))

        response = make_response({"details": {"errors": "none", "message": "Successfully created the task"} }, 200)

        response.set_cookie("appName", "TasksList", max_age=60*60*24*365)

        response.set_cookie("owner", "TBD")
